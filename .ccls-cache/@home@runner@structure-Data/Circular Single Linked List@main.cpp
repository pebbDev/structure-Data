...
void printCircular() {
  if (head == NULL) {
    ...
    while (cur->next != head) {
      // print
      cout << "| " << cur->npm << "\t| " << cur->nama << "\t| " << cur->jurusan
           << "\t\t|" << endl;
      cur = cur->next;
    }
    cout << "| " << cur->npm << "\t| " << cur->nama << "\t| " << cur->jurusan
...
int main() {
  string dataBaru[3] = {"F55121082", "Febriyadi", "Informatika"};
  createCircularSingleLinkedList(dataBaru);

  printCircular();

  string data[3] = {"F55121077", "M Rizky Fadhilah", "Informatika"};
  addFirst(data);

  printCircular();

  string data2[3] = {"F55121070", "Yunita", "Sistem Informasi"};
  addLast(data2);

  printCircular();

  string data3[3] = {"F55121064", "Ades Sinta", "Teknik Elektro"};
  addMiddle(data3, 5);
